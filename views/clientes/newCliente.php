...
<div class="modal-dialog" style="width:700px; height: 600px;">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            <div id="messageNewCliente" style="float:left; width: 400px;"></div>
            <button type="button" id="cerrarPrincipal" class="btn btn-default" data-dismiss="modal">Cerrar</button>
            <button id="saveClienteNew" type="button" class="btn btn-primary">Guardar</button>
        </div>
    </div>
</div>
...
<script type="application/javascript">
    ...
    $('#saveClienteNew').click(function(){
        var e = 'ajax.php?controller=Clientes&action=createNewCliente'; console.debug(e);
        ...
        if(nombre == '' || rut == '')
        {
            $('#messageNewCliente').html('<div class="alert alert-danger" role="alert" style="text-align: left!important;margin: 0!important;padding: 5px!important;"><strong>Error! </strong> Debes llenar al menos el nombre y rut.</div>');
        }
        else
        {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.status == "success"){
                        $('#messageNewCliente').html('<div class="alert alert-success" role="alert" style="text-align: left!important;margin: 0!important;padding: 5px!important;"><strong>Listo! </strong>' + data.message + '</div>');
                        $('#saveClienteNew').html('<i class="fa fa-check" aria-hidden="true"></i> Listo');
                        $('#modalPrincipal').hide();
                        window.location.href = "index.php?controller=Clientes&action=clientes";
                    }
                    else{
                        $('#saveClienteNew').html("Guardar");
                        $('#messageNewCliente').html('<div class="alert alert-danger" role="alert" style="text-align: left!important;margin: 0!important;padding: 5px!important;"><strong>Error! </strong>' + data.message + '</div>');
                    }
                },
                error: function (data) {
                    console.debug("error");
